...
//-------------------------LIBRARIES--------------------------------------------
  ...
  //Library's for long ass IMU code
  #include <SPI.h>
  #include <Wire.h>

//--------------------------ALL LOGGED VARIABLES--------------------------------
  //variables from MPU9250 IMU
    float AccelX; float AccelY; float AccelZ; //in g's
    float GyroX; float GyroY; float GyroZ; //in deg/s
    float MagX; float MagY; float MagZ; //milliGauss or mG
    float IMUTemp; //degrees C
  ...
  //Variables from Data Aq system itself
    int dataAqReady = 0; // triggers high at end of setup loop to notify dash display
  ...
  EasyTransfer ActiveAeroCanbusUARTOUT;
    ...
    UART_TRANSMIT_DATA_STRUCTURE_ACTIVE_AERO txUartDataACTIVEAERO;
      ...
      void SendAndRecieveActiveAeroData(){
        ...
        txUartDataACTIVEAERO._bun = 5.67;
...
//below from https://github.com/kriswiner/MPU9250/blob/master/MPU9250BasicAHRS.ino
  ...
  // but is much less computationally intensive---it can be performed on a 3.3 V Pro Mini operating at 8 MHz!
   ...
   // measured ones.
    ...
    {
      Serial.println("****Beginning MPU9250 setup Sequence**** ");
    ...
    //  MadgwickQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f,  my,  mx, mz);
      MahonyQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, my, mx, mz);
        ...
        delt_t = millis() - count;
        if (delt_t > 500) { // update LCD once per x milliseconds independent of read rate
      ...
      // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
        ...
        AccelX = 1000*ax;
        AccelY = 1000*ay;
        AccelZ = 1000*az; //in g's
        GyroX = gx;
        GyroY = gy;
        GyroZ = gz; //in deg/s
        MagX = mx;
        MagY = my;
        MagZ = mz; //milliGauss or mG
        IMUTemp = temperature; //degrees C
...
//----------------------SETUP: GPS----------------------------------------------
  ...
  void GatherGPSData(){
     ...
     if (millis() - timer > 10) {
       ...
       Serial.println(String(gpsSatellites) + "sat ");
...
//----------------------SETUP: ADS1115 ADC--------------------------------------
 ...
 void GatherADS1115Data(){
   ...
   Serial.println("ADS0: " + String(adc0) + " ADS1: " + String(adc1) +
                      " ADS2: " + String(adc2) + " ADS3: " + String(adc3));
 }
