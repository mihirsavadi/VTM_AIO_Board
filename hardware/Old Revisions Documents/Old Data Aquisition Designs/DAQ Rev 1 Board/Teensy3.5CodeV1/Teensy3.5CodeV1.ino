...
This code runs on the central Teensy 3.5 unit.
It logs data from all its peripheral sensors (vary from simple analog and
digital interfaces as well as various serial interfaces - i2c, UART etc).
It also Logs subsystem reported inputs via UART or analog/digital inputs.
It also outputs relevant information to the graphics system, autoshift,
telemetry, via UART.
In addition to above, it has a control sequence with various inputs (lidar sensor
+ brake pot etc) to control when logging starts and stops.

Setup code for sensors and breakout boards credited in relevant comments below.

SHIT LEFT TO DO (20Oct18):
1. establish shit to send and recieve from all the UARTS
2. make void loop work according to selective data logging plan
3. clean up and refresh datastring write to sd card

~ Mihir Savadi 20/Oct/2018
*/
...
//VARIABLES FOR DATA LOGGING
  ...
  //Variables from Active Aero subsystem (via UART 1)
    int ActiveAeroPosReport;
    int ActiveAeroFaultReport;
...
//create string for easy input into SD card. All variables from Data Logging Variables list above.
  ...
  String(gpsAltitude) + ", " + String(gpsSatellites) + String(ActiveAeroPosReport) + ", " +
  String(ActiveAeroFaultReport) + ", " + String(BrakeBiasPosReport) + ", " +
  ...
  //library setup for UART communication from https://github.com/madsci1016/Arduino-EasyTransfer/tree/master/EasyTransfer
    ...
    //stuff for Active Aero Module communication
      ...
      struct DATA_STRUCTURE_AERO{
        //PUT STUFF IN HERE TO RECEIVE AND SEND (MUST BE SAME ON OTHER END TOO!!!)
        int16_t ActiveAeroPosReport;
        int16_t ActiveAeroFaultReport;
      ...
      void SendAndRecieveActiveAeroData(){
          ...
          if(ActiveAeroUART.receiveData()){
            ...
            //then do whatever you want with variables inside here
            ActiveAeroPosReport = ActiveAeroData.ActiveAeroPosReport;
            ActiveAeroFaultReport = ActiveAeroData.ActiveAeroFaultReport;
          }
...
void setup(){
  ...
  Serial.println("HELLO you beautiful person. Starting overall Setup Sequence. Please be patient.");
